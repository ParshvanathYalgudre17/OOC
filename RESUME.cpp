...
public:
    ...
    void viewResume() {
        string filename;
        cout << "\nEnter filename (e.g. Namita_Resume.txt): ";
    ...
    void updateResume() {
        ...
        string info;
        if (ch == 1) {
            cout << "Enter updated skills: ";
            getline(cin, info);
            if (info.empty()) throw invalid_argument("Skills cannot be empty!");
            skills = info;
        } else if (ch == 2) {
            cout << "Enter updated experience: ";
            getline(cin, info);
            if (info.empty()) info = "Fresher";
            experience = info;
        } else {
            throw invalid_argument("Invalid choice!");
        }

        ofstream nf(filename.c_str(), ios::app);
        if (!nf) throw runtime_error("Error updating file!");
